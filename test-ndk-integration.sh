...
docker run --rm gleidsonlm/gitlab-ci-android:latest java -version
echo "✓ Java test completed"
echo ""

# Test 2: Android SDK
echo "2. Testing Android SDK..."
docker run --rm gleidsonlm/gitlab-ci-android:latest sdkmanager --list | head -10
echo "✓ Android SDK test completed"
echo ""

# Test 3: NDK Environment Variables
echo "3. Testing NDK environment variables..."
docker run --rm gleidsonlm/gitlab-ci-android:latest bash -c '
...
docker run --rm gleidsonlm/gitlab-ci-android:latest cmake --version
echo "✓ CMake test completed"
echo ""

# Test 6: Ninja
echo "6. Testing Ninja installation..."
docker run --rm gleidsonlm/gitlab-ci-android:latest ninja --version
echo "✓ Ninja test completed"
echo ""

# Test 7: Clang
echo "7. Testing Clang installation..."
docker run --rm gleidsonlm/gitlab-ci-android:latest clang --version
